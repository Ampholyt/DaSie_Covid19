...
print("All values of cough:\n", df.groupby('cough').count())
print("All values of fever:\n", df.groupby('fever').count())
df = df[df['fever'] != "None"]
df = df[df['cough'] != "None"]
# removing others
...
df = df[df['age_60_and_above'] != "None"] #added for test
# print('age filtered\n', df)
...
df = df[df['gender'] != "None"] #added for test
# print("gender filtered:\n", df)

#test_indication filtering
df.loc[(df.test_indication == 'Other'),'test_indication']=2
df.loc[(df.test_indication == 'Contact with confirmed'),'test_indication']=1
df.loc[(df.test_indication == 'Abroad'),'test_indication']=0
...
df.to_csv('data/preprocessed_test.csv', index=False)
